    ...
    function patchKeyedChildren(c1, c2, container, parentComponent, parentAnchor) {
        ...
        //以下为老的比新的多的情况
            ...
            for (let index = needToPatch - 1; index >= 0; index--) { //从后往前遍历, 这样才能确认新创建的内容的位置.
                ...
                const anchor = nextIndex + 1 < c2.length ? c2[nextIndex + 1].el : null; //注: 这个地方还是很疑惑. 因为是InsertBefore, 所以传入的其实是当前节点的下一个, 插入到之前才是正确的. 判断这个c2.length主要是为了判断一旦超过就是往后加. 其实之前也是这么操作的.总结就是, 这个anchor必须要找到需要插入的店的下一个节点.
                //注:通过计算得到, 在中间对比的情况下, c2.length永远会大于nextIndex+1 . 但是在之前的单端的比较当中, 就需要这样的判定.
                if (newIndexToOldIndexMap[index] === 0) { //WRONG IN 20200519 拿错东西了, 是要拿出映射的值. 最长增长序列怎么可能有0????
                    ...
                    if (j < 0 || index !== increasingNewIndexSequece[j]) { //+1与否得到的结果似乎是一样的. 这可能是中间对比时, 总会遇到与i+1的值相同的情况
